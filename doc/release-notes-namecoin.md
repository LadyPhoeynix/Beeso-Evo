...
- The mempool now allows multiple updates of a single name (in a chain of
  ...
  [#322](https://github.com/beeso/beeso-core/pull/322).

## Version 0.18

- BIP16, CSV and Segwit will be activated at block height 475,000 on mainnet
  and 232,000 on testnet.
  See [#239](https://github.com/beeso/beeso-core/issues/239) for
  a discussion.
...
- `name_scan` now accepts an optional `options` argument, which can be used
  ...
  See [#237](https://github.com/beeso/beeso-core/issues/237)
  for more details.
...
- Previously, `createrawtransaction` supported a separate argument for creating
  ...
  [#181](https://github.com/beeso/beeso-core/issues/181).
...
- The optional "destination address" argument to `name_update` and
  ...
  In addition, one can now also specify to send beesos to addresses
  (similar to `sendmany`) in the same transaction by using the new `sendTo`
  option.
  See also the
  [basic proposal](https://github.com/beeso/beeso-core/issues/194), which
  is not yet completely implemented, and the concrete changes done in
  [#220](https://github.com/beeso/beeso-core/pull/220) and
  [#222](https://github.com/beeso/beeso-core/pull/222).
...
- `listunspent` now explicitly handles name outputs.  In particular, the coins
  ...
  [#192](https://github.com/beeso/beeso-core/issues/192).
...
- The `transferred` field in the output of `name_list` has been changed
  ...
  See the [proposal](https://github.com/beeso/beeso-core/issues/219) and
  the [implementation](https://github.com/beeso/beeso-core/pull/236).
...
- `name_new` now checks whether a name exists already and by default rejects
  ...
  [corresponding issue](https://github.com/beeso/beeso-core/issues/54).

- Names and values in the RPC interface (and to a limited degree also the REST
  interface and `beeso-tx`) can now be specified and requested in one of
  ...
  can be found in the
  [Github issue](https://github.com/beeso/beeso-core/issues/246).

- The `beeso-tx` utility has now support for creating name operations based
  on the new commands `namenew`, ` namefirstupdate` and `nameupdate`.  For the
  exact usage, see the
  [proposal](https://github.com/beeso/beeso-core/issues/147#issuecomment-402429258).

- The "magic string" used for `signmessage` and `verifymessage` has been updated
  to be specific to Beeso (previously, the one from Bitcoin was used).  This
