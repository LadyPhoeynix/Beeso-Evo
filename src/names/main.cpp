                      ...
                      TxValidationState& state, unsigned flags)
{
  ...
  /* Check that no name inputs/outputs are present for a non-Beeso tx.
     If that's the case, all is fine.  For a Beeso tx instead, there
                      ...
                      CCoinsViewCache& view, CBlockUndo& undo)
{
  ...
  /* This check must be done *after* the historic bug fixing above!  Some
     ...
     transactions *not* marked as Beeso tx.  */
  if (!tx.IsBeeso ())
    return;
