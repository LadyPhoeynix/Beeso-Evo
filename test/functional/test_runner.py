...
BASE_SCRIPTS = [
    ...
    # FIXME: Reenable with data created for Beeso.
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci, use_term_control):
    ...
    try:
        if subprocess.check_output(["pidof", "beesod"]) not in [b'']:
            print("%sWARNING!%s There is already a beesod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
