...
"""Class for beesod node under test"""
...
class TestNode():
    """A class for representing a beesod node under test.
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Bitcoin (rather than the one from Beeso) unless an explicit value
        # is given.  This makes sure that tx fees hardcoded in some tests are
        # adequate and do not need changes for Beeso.
        ...
        self.running = True
        self.log.debug("beesod started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'beesod exited with status {} during initialization'.format(self.process.returncode)))
            try:
        ...
        self._raise_assertion_error("Unable to connect to beesod")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('beesod failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "beesod should have exited with an error"
                else:
                    assert_msg = "beesod should have exited with expected error " + expected_msg
...
class TestNodeCLI():
    ...
    def __init__(self, binary, datadir):
        ...
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.beesocli')
    ...
    def send_cli(self, command=None, *args, **kwargs):
        ...
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same beeso-cli call"
        ...
        self.log.debug("Running beeso-cli command: %s" % command)
