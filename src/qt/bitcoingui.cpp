...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Beeso address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and beeso: URIs)"));
    ...
    openAction->setStatusTip(tr("Open a beeso: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Beeso command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Beeso network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
