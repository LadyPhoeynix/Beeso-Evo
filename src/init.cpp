 ...
 */
static const char* BITCOIN_PID_FILENAME = "beesod.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/beeso/beeso-core>";
    const std::string URL_WEBSITE = "<https://beeso.org/>";
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if beeso is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if beeso is started while in a temporary directory.\n",
